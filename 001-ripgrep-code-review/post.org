...
*** DONE the search buffer
    CLOSED: [2016-11-27 dom 00:01]
*** TODO ColoredTerminal
*** TODO Out
...
If the terminal then supports coloring, the ~Colored(T)~ (where ~T~ is
~T: Terminal + Send~ enum value is used, so the ~ColoredTerminal~ instance
contains a ~Terminal~. Otherwise, the ~NoColor(T::Output)~ value is selected,
that contains only a plain ~io::Write~.
