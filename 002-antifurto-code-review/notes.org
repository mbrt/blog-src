...
when there something moving. It implements the observer pattern to notify
observers of the current state. The motion detection code is pretty simple:
...
and if they reach a certain number, motion is detected.
...
** TODO RecordingController
** TODO Whatsapp notifications
