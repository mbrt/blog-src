...
I left out the utility classes like schedulers, queues, observer lists, for
example. The dark boxes represent controller classes. They are responsible for
managing a specific part of the application. Controller classes manage all the
boxes connected via a "tilted square" arrow. This means that they both own them
(so they are responsible for their lifetime) and they know how to operate them.
Red boxes don't manage anything, but they provide a functionality either for
other classes or talk to external services.
...
and clear in this way. A consequence to this is that classes can be tested
individually much more easily, since there are no cyclical dependencies, and
children don't know anything about their parents.

In the diagram you can also see what are the input and output of each class. Red
arrows are inputs, and dark arrows are outputs. You can see that I didn't
...
stopped, it can safely delete it. The ~RecordingController~ then listens to
alarm events and whenever one happens, it forwards the pictures stream directly
to the [[PictureArchive]] and the [[DropboxUploader]].
...
all the time and effort spent, I didn't want to forget it, and also wanted to
share my insights with the community.[fn:4]
...
I hope this post gave you some interesting insights into the project and maybe
inspire you some extensions, related projects or ideas. The code is open source
on GitHub, under [[https://github.com/mbrt/antifurto][mbrt/antifurto]], as I wrote earlier. I encourage you to take a
look yourself to some of the classes. You can also build it and use it as is for
your own security alarm. The deployment is kind of a pain right now, because
there are many dependencies and configuring the external services is not exactly
easy to do (Dropbox, mails, WhatsApp, etc). The documentation is also somehow
lacking; apologies for that.
